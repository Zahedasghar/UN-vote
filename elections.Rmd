...
library(tidyverse)
#library(stat20data)
 load("D:/RepTemplates/stat20/labs/08/iran.rda")
...
```{r}
iran |> group_by(province) |> mutate(prop=ahmadinejad/legitimate_votes) |> select(1:3,10) |> arrange(-prop)
```
...
```{r}
iran |> group_by(city) |> mutate(prop=mousavi/legitimate_votes) |> select(1:2,6,10) |> arrange(-prop)
```
...
```{r}
iran |>filter(mousavi>ahmadinejad,mousavi>rezai, mousavi>karrubi) 
```
...
```{r}
iran |>filter(mousavi>ahmadinejad,mousavi>rezai, mousavi>karrubi) |> arrange(-voided_votes) |> 
  summarise(total=sum(voided_votes))
...
iran |>filter(ahmadinejad>mousavi,ahmadinejad>rezai, ahmadinejad>karrubi) |> arrange(-voided_votes) |> 
  summarise(total=sum(voided_votes))
```


```{r}
# Create a new variable 'greater_than_others' based on the condition
 iran %>%
  mutate(mousavi = ifelse(mousavi>ahmadinejad & mousavi>rezai & mousavi>karrubi, "1",
                                     ifelse(ahmadinejad > mousavi &ahmadinejad >mousavi &ahmadinejad >karrubi , "0", "other")))
...
```{r}
iran |> summarise(ahmad_prop=sum(ahmadinejad)/sum(legitimate_votes)*100,mous_prop=sum(mousavi)/sum(legitimate_votes)*100) |> round(2)
```
...
idf <- iran%>%
  mutate(first_digit = as.numeric(substr(abs(ahmadinejad), 1, 1)))
...
```{r}
idf2 <- iran%>%
  mutate(first_digit = as.numeric(substr(abs(mousavi), 1, 1)))
...
# Plot the Benford's distribution using ggplot2
ggplot(digit_freq1, aes(x = as.factor(first_digit), y = percent)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  scale_x_discrete(name = "Leading Digit") +
  scale_y_continuous(name = "Frequency (%)", labels = scales::percent_format(scale = 1)) +
  labs(title = "Mousavi's Distribution", subtitle = "Frequency of Leading Digits in Data") +
  theme_minimal()
```  
